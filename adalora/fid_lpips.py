...
from AdaLoRA.loralib.adalora import SVDLinear, RankAllocator
...
def main(args):
    ...
    # Enable training for LoRA-specific parameters
    for module in unet.modules():
        if isinstance(module, SVDLinear):
            for param in module.parameters():
                param.requires_grad = True

    for module in text_encoder_one.modules():
        if isinstance(module, SVDLinear):
            for param in module.parameters():
                param.requires_grad = True

    for module in text_encoder_two.modules():
        if isinstance(module, SVDLinear):
            for param in module.parameters():
                param.requires_grad = True
    ...
    def get_adalora_config(rank, target_modules):
        return {
            "rank": rank,
            "target_modules": target_modules,
            "adaptive": True,  # Enable adaptive rank selection
        }

    # now we will add new LoRA weights to the attention layers
    unet_target_modules = ["to_k", "to_q", "to_v", "to_out.0"]
    adalora_config = get_adalora_config(rank=args.rank, target_modules=unet_target_modules)

    # Replace nn.Linear with SVDLinear in unet
    # Collect changes in a list
    changes = []
    for name, module in unet.named_modules():
        if isinstance(module, torch.nn.Linear):
            # Replace nn.Linear with SVDLinear
    ...
    # Apply similar changes to text_encoder if needed
    if args.train_text_encoder:
        # Collect changes in a list for text_encoder_one
        changes = []
        ...
        # Apply changes after the loop
        for name, new_module in changes:
            setattr(text_encoder_one, name, new_module)

        # Collect changes in a list for text_encoder_two
        changes = []
        ...
        # Apply changes after the loop
        for name, new_module in changes:
            setattr(text_encoder_two, name, new_module)
    ...
    # Print trainable parameter counts for unet and text encoders
    ...
    if args.train_text_encoder:
        print_trainable_parameters(text_encoder_one, "Text Encoder One")
        print_trainable_parameters(text_encoder_two, "Text Encoder Two")

    # Calculate and log total trainable parameters
